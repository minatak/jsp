...
public class Calendar1Command implements StudyInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 이전 월 클릭시 1월(0)은 음수가 넘어오기에, 년도를 -1로 해주고 월은 12월(11)로 세팅해준다.
		// 다음 월 클릭시 12월(11)은 13월(12)이 넘어오기에, 년도를 +1로 해주고 월은 1월(0)로 세팅해준다.
		if(mm < 0) {
			yy--;
			mm = 11;
		}
		...
		// 선택한 해당 '년/월'의 1일을 기준으로 날짜를 세팅해준다. (처음(첫) 날짜 세팅 : '년/월/1'로 세팅해준다
		calToday.set(yy, mm, 1);
		
		// 앞에서 세팅된 해당 '년/월/1'의 요일에 해당하는 값을 숫자로 가져온다. (일:1, 월:2, 화:3, 수:4 ...) : 날짜를 표시할 시작 테이블의 1일이 들어갈 첫 열을 찾기 위해서.
