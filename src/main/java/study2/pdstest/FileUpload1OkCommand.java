...
/*
 	자바 파일 업로드시 (컨트롤러에 아래 어노테이션에 환경설정 처리한다.)
	@MultipartConfig(
...
//		System.out.println("fName : " + fName);
		...
		/*
		 	...
		 	사용법 : MultipartRequest(저장소명(request), '서버에 저장될 파일의 경로(기본 위치는 무조건 홈 디렉토리(여기선 webapp)에 저장됨)', 
		 	'서버에 저장될 파일의 최대 용량', '코드변환방식', 기타옵션(파일명 중복 방지 처리)-이건 안써도 됨);
		...
		*/
		...
		int maxSize = 1024 * 1024 * 10; // 서버에 저장시킬 파일의 최대용량 : 10MByte로 제한 (1회 저장용량)
		...
		// 닉네임 받아서 찍어보기 
		// String nickName = request.getParameter("nickName"); -> 이렇게 받으면 안됨
		String nickName = multipartRequest.getParameter("nickName");
		System.out.println("nickName : " + nickName);
		
		// 백엔드 파일체크
		if(originalFileName != null && !originalFileName.equals("")) {
			request.setAttribute("message", "파일 전송이 완료되었습니다 :)");
		}
		else { // 파일명 없이 넘어왔을 경우
			request.setAttribute("message", "파일 전송에 실패했습니다.");
		}
