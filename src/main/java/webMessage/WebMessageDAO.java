...
public class WebMessageDAO {
	...
	public WebMessageVO getWebMessageContent(int idx, int mFlag) {
		...
		try { // 하나의 메소드 안에서 두개의 쿼리 돌리기 (select, update 둘 다)
			if(mFlag == 11) { // 받은 메세지에서 신규 메세지를 클릭하면 n을 r로 바꾸기
	...
	public int wmDeleteCheck(int idx, int mSw) {
		int res = 0;
		try {
			if(mSw == 11) { // mSw(=mFlag)의 값이 11은 받은 편지함에서 휴지통으로 보낸 경우
	...
	// 휴지통에 들어있는 모든 자료들을 삭제(update)처리한다. 이 때 receiveSw와 sendSw가 모두 'x' 이면 실제 자료를 삭제(delete)처리한다. 
	public int wmDeleteAll(String mid) {
		int res = 0;
			try {
				// 보낸 메세지에서의 휴지통 처리시 수행할 sql문 
