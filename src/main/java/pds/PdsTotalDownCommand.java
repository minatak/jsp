...
public class PdsTotalDownCommand implements PdsInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		fos = new FileOutputStream(zipPath + zipName); // 출력될 압축 파일의 이름 경로
		...
		// 서버에서 압축작업이 완료되면 압축 파일을 클라이언트로 전송하고 서버에 존재하는 압축파일은 삭제시킨다. 
		// 클라이언트로 파일을 전송할 시에는 HTTP 프로토콜 형식에 맞도록 헤더에 정보를 제공해줘야한다.
		String downLoadName = "";
