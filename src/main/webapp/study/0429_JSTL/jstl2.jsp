...
%>
  ...
  <h4>사용예제</h4>
  <div>1. 구구단 5단을 출력하세요</div>
  <h5> * 5단 * </h5>
  <c:forEach var="i" begin="1" end="9">
		5 * ${i} = ${5 * i}<br/>
	</c:forEach>
	<hr/>
  <div>2. 구구단 3~5단을 출력하세요(이중 for문)</div>
 ...
 %>
  ...
  <h5>7. 6번 조건에 추가 : 나이 25세 미만은 굵은 파란색 표시해서 출력하시오.</h5>
 	<c:forEach var="member" items="${members}">
		<c:forEach var="m" items="${member}">
	...
	<h5>8. 앞의 사용법 2번에 저장한 cards의 내용 중 첫번째 카드 배경색 노랑, 마지막카드 배경색은 하늘색으로 출력하시오</h5>
	<c:forEach var="card" items="${cards}" varStatus="st">
		...
		<c:if test="${!st.first && !st.last}">${card}</span>/</c:if> 
  </c:forEach>
	
	<h5>9. 앞의 사용법 2번에 저장한 cards의 내용 중 '국민카드'는 파랑색, '삼성카드'는 빨강, 첫번째 카드 배경색 노랑, 마지막카드 배경색은 하늘색으로 출력하시오</h5>
</div>
<p><br/></p>
