...
<head>
  ...
  <script>
  	...
  	function deleteCheck() {
			let ans = confirm("회원을 탈퇴시키겠습니까? 탈퇴 후 복구는 불가능합니다.");
			if(ans) {
...
<div class="container">
  <form name="myform" method="post" action="${ctp}/database/UpdateOk">
    <table class="table table-bordered text-center">
      ...
      <tr>
        <th>아이디</th>
        <td><input type="text" name="mid" value="${vo.mid}" placeholder="아이디를 입력하세요" autofocus required class="form-control"/></td>
      </tr>
      <tr>
        <th>비밀번호</th>
        	...
        	<div class="row">
        		<div class="col text-left">
		          ...
		          <c:if test="${sMid == vo.mid}">
		          	<input type="reset" value="탈퇴" onclick="deleteCheck()" class="btn btn-warning mr-2"/>
		          </c:if>
		          <c:if test="${sMid != vo.mid}">
