...
<div class="container">
  ...
  <pre>
  	...
  	이때의 데이터가 주고받는 작업단위를 트랜젝션(Transaction)이라고 한다.
  	RequestDispatch 인터페이스는 클라이언트의 요청에 의해 컨테이너에 생성된 request와 response를 다른 리소스 (서블릿, html, jsp, 이미지)
  	로 넘겨주는 역할을 한다.
  	이런 역할을 직접 수행하는 메소드가 RequestDispatch 인터페이스의 forward() 메소드이다.
  	HTTP 통신의 응답(Response)이 완료되기 전에 서버에 계속해서 Request 할 수 있는데, 이것은 모두 컨테이너에 담겨있고
