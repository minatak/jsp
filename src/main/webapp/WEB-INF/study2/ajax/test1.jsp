  ...
  <script>
    ...
    function idCheck2() {
    	...
    	fetch("${ctp}/ajaxIdCheck1.st?mid="+mid)
    		// .then((res) => res.json())  // 지금 이렇게 처리하면 에러남. 값을 json 형식으로 넘긴게 아니기때문에 !
    ...
    // 비동기식 처리 (aJax 사용)
    	...
    	$.ajax({ 
    		...
    		type : "get", // 앞의 url, type, data 등은 예약어. 이 이름을 바꾸면 안됨. 
    		data : {"mid" : mid},
    		// dataType : "json", - dataType과 contextType을 둘다 이렇게 적으면 에러남. 둘 중 하나는 포기해야 함
    		contextType : "application/json",
    		charset : "utf-8",
    		timeout : 10000, // 10000초동안 대기하고 있어라 !
    		beforSend : function() {
    			consol.log("mid(전송전) : ", mid);
    		},
    		success:function(res) { // 정상적으로 돌아왔을 때
    			let str = "<font color='blue'>검색 아이디 : " + mid + ", 성명 : " + res + "</font>";
    			$("#demo").html(str);
    		},
    		error : function() { // 에러가 났을 때
    			alert("전송오류");
    		},
    		complete : function() { // 정상적으로 돌아오던 정상적이지 않던 무조건 실행됨
    			consol.log("mid(전송후) : ", mid)
    		}
    	});
		}
	...
	<div>
	  <form name="myform">
	    ...
	    <div>
	      ...
	      <input type="button" value="아이디검색(비동기식 1)" onclick="idCheck1()" class="btn btn-primary mt-2 mr-2"/>
	  ...
	  <hr/>
	    <div id="demo">출력결과 : <font color="blue"><b>${param.name}</b></font></div>
	  <hr/>
	</div>
	<hr/>
  <h2>HTTP통신</h2>
