...
create table review(
	...
	part 		varchar(20) not null,					/* 분야 (게시판 : board, 자료실 : pds 등) */
...
select * from review where partIdx = 5;

select * from reviewReply order by replyIdx desc;

select * from review v, reviewReply r where v.partIdx=5 and v.idx=r.reviewIdx;
select * from review v, reviewReply r where v.partIdx=5 and v.idx=r.reviewIdx order by v.idx desc, r.replyIdx desc;
select * from review v left join reviewReply r on v.partIdx=5 and v.idx=r.reviewIdx order by v.idx desc, r.replyIdx desc;
select * from (select * from review where partIdx=14) as v left join reviewReply r on v.partIdx=5 and v.idx=r.reviewIdx order by v.idx desc, r.replyIdx desc;
